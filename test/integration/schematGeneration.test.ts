...
describe('schemat generation integration testing', () => {
    describe('postgres', () => {
        ...
        it('Camelcase (tables only) generation', async () => {
            const inputSQLFile = 'test/fixture/postgres/osm.sql'
            const outputFile = './test/actual/postgres/osm-camelcase-tables.ts'
            const expectedFile = './test/expected/postgres/osm-camelcase-tables.ts'
            const config: any = './fixture/postgres/osm-camelcase-tables.json'
            await writeTsFile(inputSQLFile, config, outputFile, db)
            return assert(await compare(expectedFile, outputFile))
        })
